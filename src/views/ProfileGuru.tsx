...
export const ProfileGuru: React.FC = () => {
  const currentUser = db.getCurrentUser();
  if (!currentUser) return null;

  const [nama, setNama] = useState(currentUser.nama);
  const [email, setEmail] = useState(currentUser.email);
  const [nip, setNip] = useState(currentUser.nip_nisn);
  ...
  const handleUpdateProfile = (e: React.FormEvent) => {
    ...
    try {
      const updatedUser = db.updateCurrentUser({
        nama,
        email,
        hp,
        photo_url: photoUrl
      });
      setStatusMsg("Profil Guru berhasil disimpan!");
      // Reload page or trigger profile update context if needed
    } catch (err: any) {
    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div className="bg-white dark:bg-[#1f202e] border border-gray-100 dark:border-gray-800 rounded-2xl p-6 flex flex-col items-center text-center justify-between">
        <div className="w-full flex flex-col items-center">
          <div className="relative mb-4">
            <img
              src={photoUrl || "https://images.unsplash.com/photo-1573496359142-b8d87734a5a2?q=80&w=200&auto=format&fit=crop"}
              alt={nama}
          ...
          <span className="text-xs px-2.5 py-0.5 rounded-full bg-indigo-50 dark:bg-indigo-950/40 text-indigo-600 dark:text-indigo-400 font-bold uppercase tracking-wider text-[10px] mt-1.5">
            Guru Utama
          </span>
          ...
          <div className="w-full border-t border-gray-100 dark:border-gray-800 my-5 pt-5 space-y-4 text-left text-xs text-gray-500 dark:text-gray-400">
            <div className="flex justify-between">
              <span className="font-medium">NIP / NUPTK</span>
              <span className="font-mono text-gray-800 dark:text-gray-200 font-semibold">{nip}</span>
            </div>
            <div className="flex justify-between">
              <span className="font-medium">Email Terdaftar</span>
        ...
        <div className="w-full text-[10px] text-gray-400 font-mono text-center pt-4 border-t border-gray-50 dark:border-gray-800/40">
          Akun dibuat: {new Date(currentUser.created_at || "").toLocaleDateString("id-ID")}
        </div>
      </div>
      ...
      <div className="lg:col-span-2 space-y-6">
        ...
        <div className="bg-white dark:bg-[#1f202e] border border-gray-100 dark:border-gray-800 rounded-2xl p-6 shadow-sm">
          <h3 className="text-base font-bold text-gray-900 dark:text-white mb-1">Pengaturan Profil</h3>
          <p className="text-xs text-gray-400 mb-6">Kelola informasi data diri Anda di Aliyah Al-Qamar.</p>
          ...
          <form onSubmit={handleUpdateProfile} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-xs font-semibold text-gray-500 dark:text-gray-400 mb-1.5 uppercase tracking-wider">Nama Lengkap</label>
              ...
              <div>
                <label className="block text-xs font-semibold text-gray-500 dark:text-gray-400 mb-1.5 uppercase tracking-wider">NIP / NUPTK</label>
                <div className="relative">
                  <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-gray-400">
                    <ShieldCheck size={15} />
                  </div>
                  <input
                    ...
                    disabled
                    value={nip}
                    className="w-full pl-9 pr-4 py-2.5 text-xs bg-gray-50 dark:bg-slate-950/50 border border-gray-200 dark:border-gray-850 rounded-xl text-gray-400 dark:text-gray-600 cursor-not-allowed"
              ...
              <div>
                <label className="block text-xs font-semibold text-gray-500 dark:text-gray-400 mb-1.5 uppercase tracking-wider">Nomor Handphone</label>
                <div className="relative">
                  <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-gray-400">
                    <Phone size={15} />
                  </div>
        ...
        <div className="bg-white dark:bg-[#1f202e] border border-gray-100 dark:border-gray-800 rounded-2xl p-6 shadow-sm">
          ...
          <p className="text-xs text-gray-400 mb-6">Pastikan password Anda memiliki keamanan yang kuat.</p>
